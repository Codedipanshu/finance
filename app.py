    ...
    if request.method == "POST":
        ...
        if total_price > cash:
            ...
            db.execute("INSERT INTO transactions (user_id, symbol, name, price, shares, type) VALUES (?, ?, ?, ?, ?, ?)",
                       user_id, symbol, item_name, item_price, shares, 'buy')
...
def history():
    ...
    user_id = session["user_id"]
    stocks = db.execute("SELECT symbol, name, shares, price, time FROM transactions WHERE user_id = ?", user_id)
...
def register():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            ...
            return apology("passward and confirmation must same")
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, name, price, shares, type) VALUES (?, ?, ?, ?, ?, ?)",
                   user_id, symbol, item_name, item_price, -shares, 'sell')
